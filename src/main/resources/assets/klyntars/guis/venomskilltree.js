...
PalladiumEvents.renderPowerScreen(event => {
    ...
    if (event.tab.toString().includes("klyntars:")) {
        let power = event.tab.toString().replace("klyntars:", "")
        let colour = palladium.getProperty(entity, `klyntar.${power}Colour`)
        let size = 257
        event.guiGraphics.blit(new ResourceLocation(`klyntars:textures/gui/skilltrees/${power}/hud${colour}.png`),
