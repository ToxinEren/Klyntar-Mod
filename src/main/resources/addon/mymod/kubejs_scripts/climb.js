  ...
  event.create('mymod:climb')
    ...
    .tick((entity, entry, holder, enabled) => {
      if (enabled && entity.isPlayer()) {
        ...
        let isBlockFrontSolid = blockFront && !blockFront.hasTag('mymod:block_pass') && isCloseToBlock(entity, blockFront, block_dist);
        let isBlockBehindSolid = blockBehind && !blockBehind.hasTag('mymod:block_pass') && isCloseToBlock(entity, blockBehind, block_dist);
        let isBlockRightSolid = blockRight && !blockRight.hasTag('mymod:block_pass') && isCloseToBlock(entity, blockRight, block_dist);
        let isBlockLeftSolid = blockLeft && !blockLeft.hasTag('mymod:block_pass') && isCloseToBlock(entity, blockLeft, block_dist);
        let isBlockAboveSolid = blockAbove && !blockAbove.hasTag('mymod:block_pass') && isCloseToBlock(entity, blockAbove, 2.5);
        let isBlockBelowSolid = blockBelow && !blockBelow.hasTag('mymod:block_pass');
...
StartupEvents.registry('palladium:condition_serializer', (event) => {
  event.create('mymod:is_climb_stage')
    .addProperty('data', 'string', 'oms.climb', 'persistent data name')
