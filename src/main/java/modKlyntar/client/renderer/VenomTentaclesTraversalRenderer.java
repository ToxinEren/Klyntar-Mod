...
public final class VenomTentaclesTraversalRenderer {
    ...
    private static void renderArmPath(Vec3 start, Vec3 end, int armIndex, PoseStack poseStack, MultiBufferSource buffer, long gameTime) {
        disegnaTentacolo(start, end, armIndex, armTexture, poseStack, buffer, gameTime);
        livingArmorSulTentacolo(start, end, armIndex, poseStack, buffer, gameTime);
    }
    ...
    /**
     * La living armor sui bracci della traversata.
     *
     * <p>I punti non sono stimati: si campiona la STESSA formula con cui
     * {@link #disegnaTentacolo} costruisce la curva — avanzamento lungo delta piu' l'onda
     * laterale — quindi ogni radice cade esattamente sul braccio, qualunque cosa faccia
     * l'animazione. E' l'unico modo per non doverli riallineare a occhio.</p>
     *
     ...
     */
    private static void livingArmorSulTentacolo(Vec3 start, Vec3 end, int armIndex,
        ...
        Vec3 sideWave = lato.scale(0.06D);
        ...
        for (int k = 0; k < FILAMENTI_PER_BRACCIO; k++) {
            ...
            // la stessa onda di disegnaTentacolo, alla stessa progressione
            ...
            Vec3 sulBraccio = start.add(delta.scale(progress)).add(sideWave.scale(wave));
            ...
            Vec3 radice = sulBraccio.add(fuori.scale(TENTACLE_RADIUS * 0.6D));
            Vec3 punta = sulBraccio.add(fuori.scale(TENTACLE_RADIUS + FILAMENTO_LUNGHEZZA * respiro));

            disegnaFilamento(radice, punta, seme, armTexture,
                    TENTACLE_RADIUS * 0.22F, TENTACLE_RADIUS * 0.05F, 8, 0.05D,
                    poseStack, buffer, gameTime);
        }
    }
